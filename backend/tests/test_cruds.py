...
from app.models import InvoiceAmount, Patient, User  # noqa: F401
...
class CrudTestCase(unittest.TestCase):
    ...
    def test_patient_create_search_and_update(self):
        patient_crud.create_patient(
            self.db,
            PatientCreate(name="山田 太郎", affiliation="青空薬局", memo="毎月請求"),
        )
        ...
        updated = patient_crud.update_patient(
            self.db,
            patient,
            PatientUpdate(name="山田 太郎", affiliation="青空病院", memo=""),
        )
    ...
    def test_invoice_totals_and_previous_month_copy(self):
        patient = patient_crud.create_patient(self.db, PatientCreate(name="佐藤 花子"))
        ...
        self.assertEqual(sum(item.amount for item in april.patients[0].amounts), 2000)

        may = invoice_crud.get_or_create_invoice(self.db, 2026, 5)
        self.assertEqual([item.patient_id for item in may.patients], [patient.id])
        self.assertEqual(may.patients[0].amounts, [])

    def test_password_hash_and_token(self):
        hashed = hash_password("safe-password")
        self.assertNotEqual(hashed, "safe-password")
        self.assertTrue(verify_password("safe-password", hashed))
        self.assertFalse(verify_password("wrong", hashed))
        self.assertIsInstance(create_access_token({"sub": "staff"}), str)
