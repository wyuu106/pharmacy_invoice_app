...
class Invoice(Base):
    ...
    patients = relationship(
        "InvoicePatient", back_populates="invoice", cascade="all, delete-orphan", order_by="InvoicePatient.id"
    )


class InvoicePatient(Base):
    __tablename__ = "invoice_patients"
    __table_args__ = (UniqueConstraint("invoice_id", "patient_id", name="uq_invoice_patient"),)
    ...
    amounts = relationship(
        "InvoiceAmount", back_populates="invoice_patient", cascade="all, delete-orphan", order_by="InvoiceAmount.id"
    )
